...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Mirrors src/lib/planLimits.ts — keep both in sync (separate Deno deploy, no shared import).
    const TRIAL_LIMIT = 10
    const ESSENTIEL_LIMIT = 20
    const CROISSANCE_LIMIT = 50
    ...
    if (isCroissance && quotesThisMonth >= CROISSANCE_LIMIT) {
      return new Response(JSON.stringify({ success: false, error: 'quota_exceeded' }), {
        status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
