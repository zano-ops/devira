      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;max-width:600px;">
        ...
        <tr><td style="padding:32px 40px;">
          ...
          <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 20px;">
            Veuillez trouver ci-joint votre devis <strong>${quote.quote_number}</strong> pour les travaux
            <strong>${quote.quote_json?.titre || ''}</strong>.
          </p>

          <table width="100%" style="background:#F0F4F8;border-radius:8px;padding:16px;">
            <tr>
            ...
            <tr>
              <td style="color:#6B7280;font-size:13px;padding-top:8px;">Montant TTC</td>
              <td style="color:#1E3A5F;font-weight:bold;font-size:18px;text-align:right;padding-top:8px;">
                ${(quote.total_ttc || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €
              </td>
            </tr>
          </table>

          <p style="color:#6B7280;font-size:14px;margin:20px 0 8px;">
            Ce devis est valable <strong>${quote.quote_json?.validite_jours || 30} jours</strong>.
          </p>

          ${quote.quote_json?.notes ? `<p style="color:#6B7280;font-size:13px;font-style:italic;margin:0;">${quote.quote_json.notes}</p>` : ''}
